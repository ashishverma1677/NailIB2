...
$result2 = mysqli_query($mysqli, "SELECT * FROM student where username='$username'"); 
while($row2 = mysqli_fetch_array($result2)) { 	
	echo "<td><a href=\"Personalprofile2.php?stu_id=$row2[stu_id]\" style='text-decoration: none;
    color: black;'><span style='
    background-color: #3BCEFC;
    border: none;
    color: white;
    padding: 15px 45px;
    text-align: center;
...
>Edit</span>
    ...
    ?>
		<!--<button class="buttonVerifyEmail">Edit</button>-->
		</div>	
...
$_SESSION["stu_id"] =$stu_id;
		...
		$chg_pwd=mysql_query("select * from student where stu_id='" . $_SESSION["stu_id"] . "'");
		$chg_pwd1=mysql_fetch_array($chg_pwd);
		$data_pwd=$chg_pwd1['password'];
		if($data_pwd==$old_pass){
		if($new_pass){
			$update_pwd=mysql_query("update student set password='$new_pass' where stu_id='" . $_SESSION["stu_id"] . "'");
			echo "<script>alert('Update Sucessfully'); window.location='Personalprofile.php'</script>";
		}
		else{
			echo "<script>alert('Your new and Retype Password is not match'); window.location='Personalprofile.php'</script>";
		}
		}
		else
		{
		echo "<script>alert('Your old password is wrong'); window.location='Personalprofile.php'</script>";
		}}
	?>
